...
- [ ] `BRIEF.md` created with comprehensive autopsy of all 6 inspiration links and design system rules.
- [ ] `ASSETS.md` created with exact Higgsfield AI prompts for all media components.
- [ ] Self-contained static site (`index.html`, `css/style.css`, `js/main.js`, `assets/`) built in working directory.
- [ ] Zero JS console errors on page load and during scroll interactions.
- [ ] GSAP ScrollTrigger and Lenis smooth scrolling operate without layout shifts or stutters.
- [ ] Pinned 360° orbit video scrub section responds smoothly to scroll progress.
- [ ] Responsive down to 360px viewport width with no overflow.
- [ ] `prefers-reduced-motion` and `?noloader` correctly bypass/simplify animations.
</USER_REQUEST>
